...
include { MULTIQC_MAPPINGS_CONFIG } from '../../../modules/local/fetchngs/multiqc_mappings_config/main'
...
include { ASPERA_CLI              } from '../../../modules/local/fetchngs/aspera_cli/main'
include { SRA_TO_SAMPLESHEET      } from '../../../modules/local/fetchngs/sra_to_samplesheet/main'
...
workflow FETCHNGS {
    ...
    if (!params.skip_fastq_download) {
        ...
        // NOTE: this repo's installed FASTQ_DOWNLOAD_PREFETCH_FASTERQDUMP_SRATOOLS subworkflow
        // (git_sha 8e6139d) migrated its underlying sra-tools modules to nf-core topic-channel
        // version reporting and no longer emits a `versions` channel (only `reads`), unlike
        // upstream fetchngs 1.12.0's pinned copy. Those tool versions are instead collected
        // automatically via `channel.topic('versions')` at the top-level pipeline, matching
        // this repo's existing convention in workflows/plastier.nf. See task report for detail.
    ...
    softwareVersionsToYAML(ch_versions)
        .collectFile(storeDir: "${params.outdir}/pipeline_info", name: 'plastier_fetchngs_software_mqc_versions.yml', sort: true, newLine: true)
